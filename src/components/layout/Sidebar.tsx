...
const navigation: NavItem[] = [
  ...
  },
  {
    ...
    icon: (
      <svg className="w-5 h-5" width="20" height="20" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.75 3.104v5.714a2.25 2.25 0 01-.659 1.591L5 14.5M9.75 3.104c-.251.023-.501.05-.75.082m.75-.082a24.301 24.301 0 014.5 0m0 0v5.714a2.25 2.25 0 00.659 1.591L19 14.5M14.25 3.104c.251.023.501.05.75.082M19 14.5l-1.43 4.294a2.25 2.25 0 01-2.134 1.556H8.564a2.25 2.25 0 01-2.134-1.556L5 14.5m14 0H5" />
      </svg>
    ),
  },
  {
...
/* ── Tour trigger button (rendered inside nav) ──────────────────── */
...
export default function Sidebar({ user, onSignOut }: SidebarProps) {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        {navigation.map((item) => (
          <Link
            key={item.name}
            href={item.href}
            data-tour={`tour-nav-${item.name.toLowerCase()}`}
      ...
      {/* User Section */}
      <div className="border-t border-white/10 p-4" data-tour="tour-user-profile">
